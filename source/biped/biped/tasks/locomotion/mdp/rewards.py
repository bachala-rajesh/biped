...
if TYPE_CHECKING:
    ...
    from isaac.lab.managers import RewardTermCfg


########
# rewards taken from git repository: https://github.com/Andy-xiong6/bipedal_locomotion_isaaclab
########
...
########
# rewards taken from SF_biped issacgym
########

def base_height_l2(env: ManagerBasedRLEnv, target_height: float, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Penalize base height deviation from a fixed target."""
    # Legacy: torch.square(base_height - self.cfg.rewards.base_height_target)
    asset: Articulation = env.scene[asset_cfg.name]
    base_height = asset.data.root_pos_w[:, 2]
    return torch.square(base_height - target_height)

def joint_pos_target_manual(
    env: ManagerBasedRLEnv, 
    left_joint_name: str, 
    ...
    """
    Penalize specific joints deviating from manual targets.
    Replaces legacy '_reward_leeg_roll' which hardcoded indices 1 and 4.
    """
    asset: Articulation = env.scene[asset_cfg.name]
    
    # Get indices for the specific joints
    # Note: We assume the user provides specific regex that matches exactly one joint per arg
    left_idx = asset.find_joints(left_joint_name)[0]
    right_idx = asset.find_joints(right_joint_name)[0]
    
    # Current positions
    current_pos = asset.data.joint_pos
    
    l_roll_error = torch.abs(current_pos[:, left_idx] - left_target)
    r_roll_error = torch.abs(current_pos[:, right_idx] - right_target)
    
    return l_roll_error + r_roll_error


def feet_air_time(
    env: ManagerBasedRLEnv, command_name: str, sensor_cfg: SceneEntityCfg, threshold: float
) -> torch.Tensor:
    """Reward long steps (air time) for feet.
    
    This function rewards the robot for keeping feet in the air, encouraging longer strides.
    It only applies when the robot is commanded to move (velocity > 0.1).
    """
    # 1. Get the contact sensor
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    
    # 2. Get the command (to check if we should be moving)
    #    Legacy code: torch.norm(self.commands[:, :2], dim=1) > 0.1
    cmd = env.command_manager.get_command(command_name)
    is_moving = torch.norm(cmd[:, :2], dim=1) > 0.1
    
    # 3. Get current air time
    #    'current_air_time' is a counter that resets to 0 when foot touches ground.
    #    We access it only for the specific feet bodies defined in sensor_cfg.
    air_time = contact_sensor.data.current_air_time[:, sensor_cfg.body_ids]
    
    # 4. Compute Reward
    #    Reward = (air_time - threshold) summed over all feet
    reward = (air_time - threshold).clamp(min=0.0)
    
    return torch.sum(reward, dim=1) * is_moving